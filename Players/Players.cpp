...
namespace Engine
{
	...
	void CPlayers::Update()
	{
		...
		for (BYTE EntIndex = 0; EntIndex < GetSize(); EntIndex++)
		{
			...
			if (!pEntity || !pEntity->IsPlayer() || !pEntity->IsValid() || pEntity == m_pMe->m_pEntity|| EntIndex == m_pMe->iIndex || pEntity->GetClientClass()->m_ClassID != (int)CLIENT_CLASS_ID::CCSPlayer)
			{
			...
			if (Esp)
			{
				vHead.z += settings.Visual.esp_Size;
				vOrigin.z -= settings.Visual.esp_Size;
			}
			...
			if (Esp && settings.Visual.CHvisual[changvis].esp_Skeleton && m_pMe->bAlive)
			{
				// BODY
			...
			if (Esp && settings.Visual.esp_BulletTrace)
			{
				...
				Math::AngleVectors(pEntity->GetRenderAngles(), &vForward, 0,0);
				filter.pSkip = pEntity;

				vScr = pEntity->GetHitboxPosition(HITBOX_HEAD);
				vDst = vScr + (vForward * (float)settings.Visual.esp_BulletTrace);
