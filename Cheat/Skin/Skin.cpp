...
// So much messy code here oh jesus ~ Baseult
...
{
	int iKnifeModelID = (tt ? settings.SkinChanger.knf_tt_model - 1 : settings.SkinChanger.knf_ct_model - 1);
...
void CSkin::SetSkinConfig()
{
	...
	if (settings.SkinChanger.knf_ct_model >= 1 && settings.SkinChanger.knf_ct_model <= 14)
	{
		g_SkinChangerCfg[WEAPON_KNIFE].iItemDefinitionIndex = KnifeModelsType[settings.SkinChanger.knf_ct_model - 1];
	}
	else
	{
	...
	if (settings.SkinChanger.knf_ct_skin)
	{
		g_SkinChangerCfg[WEAPON_KNIFE].nFallbackPaintKit = settings.SkinChanger.knf_ct_skin;
	}
	else
	{
	...
	if (settings.SkinChanger.knf_tt_model >= 1 && settings.SkinChanger.knf_tt_model <= 14)
	{
		g_SkinChangerCfg[WEAPON_KNIFE_T].iItemDefinitionIndex = KnifeModelsType[settings.SkinChanger.knf_tt_model - 1];
	}
	else
	{
	...
	if (settings.SkinChanger.knf_tt_skin)
	{
		g_SkinChangerCfg[WEAPON_KNIFE_T].nFallbackPaintKit = settings.SkinChanger.knf_tt_skin;
	}
	else
	{
...
void CSkin::SetModelConfig()
{
	...
	if (settings.SkinChanger.knf_ct_model >= 1 && settings.SkinChanger.knf_ct_model <= 14)
	{
		char* mdl_ct = pszKnifeModels[settings.SkinChanger.knf_ct_model - 1];
	...
	if (settings.SkinChanger.knf_tt_model >= 1 && settings.SkinChanger.knf_tt_model <= 14)
	{
		char* mdl_tt = pszKnifeModels[settings.SkinChanger.knf_tt_model - 1];
...
void CSkin::SetKillIconCfg()
{
	...
	if (settings.SkinChanger.knf_ct_model >= 1 && settings.SkinChanger.knf_ct_model <= 14)
	{
		g_KillIconCfg["knife_default_ct"] = pszKnifeModelsIcon[settings.SkinChanger.knf_ct_model - 1];
	}
	else
	{
	...
	if (settings.SkinChanger.knf_tt_model >= 1 && settings.SkinChanger.knf_tt_model <= 14)
	{
		g_KillIconCfg["knife_t"] = pszKnifeModelsIcon[settings.SkinChanger.knf_tt_model - 1];
	}
	else
	{
...
{
	if (!settings.SkinChanger.gloves_skin)
		return;
	...
	if (!Interfaces::EntityList()->GetClientEntityFromHandle((PVOID)pWareables[0]))
	{
		...
		*pGlove->GetItemDefinitionIndex() = GlovesSkin_Array[settings.SkinChanger.gloves_skin - 1].ItemIndex;
		...
		*pGlove->GetFallbackPaintKit() = GlovesSkin_Array[settings.SkinChanger.gloves_skin - 1].PaintKit;

		int ModelIndex = Interfaces::ModelInfo()->GetModelIndex(
			GlovesSkin_Array[settings.SkinChanger.gloves_skin - 1].szModel);
...
bool CustomPrecacheModel(const char* szModelName)
{
	INetworkStringTable* m_pModelPrecacheTable = Interfaces::ClientStringTableContainer()->FindTable("modelprecache");
...
//coded by Smixy, give him some reputation :). Well done mate ~ baseult
void Model_OnFrameStageNotify(ClientFrameStage_t Stage)
{
	...
	CBaseWeapon* pWeapon = (CBaseWeapon*)Interfaces::EntityList()->GetClientEntityFromHandle(local->GetActiveWeapon());

	if (Stage == ClientFrameStage_t::FRAME_RENDER_START && Interfaces::Engine()->IsInGame())
	{
		if (settings.Misc.custommodelson)
		{
				...
				switch (settings.Misc.customweaponmodelknife) // For knives
				{
				case 0:
					break;
				case 1:
				{
					CustomPrecacheModel("models/weapons/v_minecraft_pickaxe.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/v_minecraft_pickaxe.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
					if (local->GetWeapon()->IsKnife())
					{
						pViewModel->SetModelIndex(knife);
					}
					break;
				}
				case 2:
				{
					CustomPrecacheModel("models/weapons/ventoz/Abyss_Greatsword/v_abyss_greatsword.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/ventoz/Abyss_Greatsword/v_abyss_greatsword.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				case 3:
				{
					CustomPrecacheModel("models/weapons/eminem/sharpened_volcano_fragment/v_sharpened_volcano_fragment.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/sharpened_volcano_fragment/v_sharpened_volcano_fragment.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				case 4:
				{
					CustomPrecacheModel("models/weapons/eminem/fidget_spinner/v_fidget_spinner.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/fidget_spinner/v_fidget_spinner.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				case 5:
				{
					CustomPrecacheModel("models/weapons/eminem/adidas_baseball_bat/v_adidas_baseball_bat.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/adidas_baseball_bat/v_adidas_baseball_bat.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				case 6:
				{
					CustomPrecacheModel("models/weapons/eminem/player_screwdriver/v_player_screwdriver.mdl");
					int knife = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/player_screwdriver/v_player_screwdriver.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				switch (settings.Misc.AK47Models) // For AK47
				{
				case 0:
					break;
				case 1:
				{
					CustomPrecacheModel("models/weapons/v_rif_ak47_transformers.mdl");
					int gun = Interfaces::ModelInfo()->GetModelIndex("models/weapons/v_rif_ak47_transformers.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
					if (local->GetWeapon()->IsAK47())
					{
						pViewModel->SetModelIndex(gun);
					}
					break;
				}
				}

				switch (settings.Misc.AWPModels) // For AWP
				{
				case 0:
					break;
				case 1:
				{
					CustomPrecacheModel("models/weapons/eminem/dsr_50/v_dsr_50_2.mdl");
					int gun = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/dsr_50/v_dsr_50_2.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
					if (local->GetWeapon()->IsAWP())
					{
						pViewModel->SetModelIndex(gun);
					}
					break;
				...
				case 2:
				{
					CustomPrecacheModel("models/weapons/eminem/crysis3_predator_bow/v_crysis3_predator_bow.mdl");
					int gun = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/crysis3_predator_bow/v_crysis3_predator_bow.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
				...
				switch (settings.Misc.GlockModels) // For Glock
				{
				case 0:
					break;
				case 1:
				{
					CustomPrecacheModel("models/weapons/v_glock18_gspirit.mdl");
					int gun = Interfaces::ModelInfo()->GetModelIndex("models/weapons/v_glock18_gspirit.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
					if (local->GetWeapon()->IsGlock())
					{
						pViewModel->SetModelIndex(gun);
					}
					break;
				...
				switch (settings.Misc.M4A4Models) // For M4A4
				{
				case 0:
					break;
				case 1:
				{
					CustomPrecacheModel("models/weapons/eminem/ethereal/v_ethereal.mdl");
					int gun = Interfaces::ModelInfo()->GetModelIndex("models/weapons/eminem/ethereal/v_ethereal.mdl");
					CBaseViewModel* pViewModel = local->GetViewModel();
					if (!pViewModel && local->IsDead())
						return;
					DWORD hViewModelWeapon = pViewModel->GetWeapon();
					if (local->GetWeapon()->IsM4A4())
					{
						pViewModel->SetModelIndex(gun);
					}
					break;
